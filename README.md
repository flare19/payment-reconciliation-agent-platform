...
| **Deep links to new runs don't survive a refresh** | `/?run=<id>` cold-loads correctly for runs that existed at build time and hangs on a loading skeleton for runs created since. The pinned dashboard and the in-page "Run It Again" flow are unaffected; a shared or bookmarked link to a fresh run is not. Found on 2026-09-04, unfixed at freeze — a redeploy this close to submission is the larger risk. |
| **The Analyst UI says "nine tools"; there are eleven** | Copy written before [ADR-159](docs/adr-log.md) and [ADR-171](docs/adr-log.md) added the tenth and eleventh. The registry is correct and the eleventh is demonstrably live. Same reason as above for not shipping a one-word redeploy. |
