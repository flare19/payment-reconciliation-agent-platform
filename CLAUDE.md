...
On the **live deploy** the record is 3 investigations: 2 concluded with grounding passed (one `NEEDS_EXTERNAL_DATA`, one `CONFIRMED_UNRESOLVABLE` after 6 tool calls), 1 **failed** at the 2,048-token output ceiling — thinking tokens count against it — returning no verdict after $0.10. It failed loudly rather than fabricating, which is the designed behaviour, but the ceiling is a real limit on harder exceptions (what-broke Day 18). Tool counts per surface, because they differ and the UI got this wrong once: registry **11** · investigation **11** · Ask **10** (`qaRegistry` drops `rerun_subset_search`) · corroboration **9** (also drops `get_exception`).
