...
### 4.5 Explain layer
One LLM call per **discrepancy shape**, not per exception (ADR-018). The model receives a decision already made and writes prose about it; it never influences match/no-match, category or severity (ADR-017). If the LLM API is absent or failing, the run completes with template explanations. The explain layer is never on the critical path.
...
| **The LLM API is unavailable, or its free-tier quota is exhausted, during the demo.** | Templates per category; run completes; `llmConfigured` visible on `/api/health`. The explain layer is never on the critical path (ADR-017). |
