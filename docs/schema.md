...
**A pair with no shared reference of any kind can never be auto-confirmed** — at any amount, on any date, with any name similarity. This is not a tunable threshold; it falls out of the arithmetic: with anchor scored `0.00`, the ceiling is `0.70` (the table above), a full `0.05` below the `0.85` auto-confirm line. Amount-and-date agreement is a coincidence generator; a reference number is evidence.

**Whether an anchorless pair can even reach the review band is narrower than "any amount, any date" suggests, and two independent mechanisms cap it (#47, #48):**

- **bank ↔ ledger never reaches the review band, at any date.** §5.3.1: there is no comparable amount basis between these two sources, so the amount component scores `0` and is flagged `amountUnavailable` — **not renormalized** (ADR-064). An anchorless bank↔ledger pair's maximum is `0.00 (anchor) + 0.00 (amount, unavailable) + 0.20 (date) + 0.15 (counterparty) = 0.35`, below the `0.65` floor regardless of how well the date and counterparty agree. Measured on the holdout: every one of the 244 true bank↔ledger pairs is anchorless, and renormalizing the missing amount weight recovers zero of them (#47) — the caps are real, not theoretical.
- **Every other pair (gateway↔bank, gateway↔ledger) reaches the review band only on a SAME-DAY match.** The date component (`0.20 × (1 − days_off / window_span)`, §5.2) reaches exactly zero at the edge of its own window — `days_off == window_span` is still *inside* the window (a T+3 card settlement is normal, per §5.2), yet scores identically to a match a month late. With anchor `0.00` and a perfect amount and counterparty (`0.35 + 0.15 = 0.50`), clearing `0.65` needs `date ≥ 0.15`, i.e. `days_off ≤ window_span / 4`. Every window §5.2 defines has `window_span ≤ 3`, so that bound is satisfiable only at `days_off = 0`. Measured on the holdout: of the 43 gateway-bearing never-found pairs with a perfect amount, in-window date and perfect counterparty trigram, 20 score exactly `0.00` on date while sitting inside the window they were measured against.

So the honest property is: **an anchorless pair can reach the review band only with a comparable amount and a same-day match** — never for bank↔ledger, and never for any pair with even a one-day lag, however normal that lag is under its own settlement SLA. Whether the date curve's zero-at-boundary behavior should change is a separate, ADR-027-governed scoring question (a date inside its own window arguably deserves partial credit rather than none) — recovering close to none of the affected pairs even if changed, since they are still short of the `0.65` floor by the missing anchor weight — and is deliberately not decided here (ADR-091).
