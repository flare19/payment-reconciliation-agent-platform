...
### ADR-149 · The error boundary told a rate-limited reader "nothing is wrong with the API"

**A 429 was misdiagnosed as a client-side rendering bug, on the exact page it happened.** `app/error.tsx` classifies a caught error by whether `error.message` looks like an API failure; anything that doesn't match falls to *"This is a rendering error, not a connection problem... nothing is wrong with the API or your network."* `ApiClientError`'s `message` is the API's raw JSON text alone (`"Rate limit reached for read requests (120 per 60s). Retry in 7s."`) — `status` and `code` are separate class fields that the boundary's own comment already says do not survive serialization across the server/client boundary. That string contains no `[45]\d{2}` substring for the old regex to catch, so a real, API-issued refusal read as "the API is fine, the page just failed to draw" — precisely the wrong-direction diagnosis the surrounding comment says this file exists to prevent.
...
### ADR-155 · An optional name for a run, purely cosmetic, exactly as scoped

**Tejas's own framing: "purely cosmetic and doesn't really matter much either way."** A text field on the run launcher, `Name this run (optional)`, defaulting to blank; a typed name fully replaces the auto-generated `<dataset>-<timestamp>` label, and leaving it blank keeps the existing behaviour unchanged.

**Checked before building, not after, that a full override is actually safe.** The auto-generated label exists specifically so the label NAMES THE DATASET IT RAN (a real fix, `demo-<timestamp>` used to lie about reconciling the holdout). A custom name could in principle re-introduce that same lie by omission — so before adding the field, every reference to `.label` in the actual project source (not `node_modules`) was checked for anything that infers the dataset by parsing the label string. Exactly one hit, and it compares a *dataset's* own label from `/api/health` (always literally `"holdout"` or `"demo"`) — nothing compares a *run's* label. Every screen that needs to know which dataset a run reconciled reads `datasetSeed`, a real field, never the label. A custom name is therefore safe to be a full override: it cannot hide the dataset from anything that actually depends on knowing it.
