...
| LLM | **Anthropic API**, `claude-sonnet-5` (ADR-093; `LLM_PROVIDER=gemini` restores the free-tier Gemini path of ADR-080 as a manual fallback) | Called only from the API service. |
...
| `LLM_PROVIDER` | `anthropic` | no | `anthropic` \| `gemini`, default `anthropic` (ADR-093). Selects the provider for BOTH LLM surfaces at once — never set per-surface; see `services/llm-provider.ts`'s header comment for why. |
| `ANTHROPIC_API_KEY` | `sk-ant-…` | no* | *Absent under the default provider → explain layer degrades to templates, Phase A returns 503, and the run still completes (ADR-017, ADR-093). One key serves BOTH layers. |
| `GEMINI_API_KEY` | `AIza…` | no | Only read when `LLM_PROVIDER=gemini` — a manual fallback path, not automatic failover if the Anthropic key stops working mid-run. Restores the Day 12 Gemini path exactly (ADR-093). |
| `LLM_EXPLAIN_MODEL` | `claude-sonnet-5` | no | Defaults in code, following `LLM_PROVIDER`. Overridable without a deploy; a change invalidates `explanation_cache` by design (ADR-018). Under `LLM_PROVIDER=gemini`, `GEMINI_EXPLAIN_MODEL` (default `gemini-3.5-flash`) is read instead. |
| `LLM_AGENT_MODEL` | `claude-sonnet-5` | no | Defaults in code, following `LLM_PROVIDER`. Phase A only. Under `LLM_PROVIDER=gemini`, `GEMINI_AGENT_MODEL` (default `gemini-3.7-flash`) is read instead. |
| `AGENT_EFFORT` | `high` | no | Anthropic reasoning depth (`output_config.effort`) — the ADR-093 replacement for the `temperature: 0` Gemini used. No effect under `LLM_PROVIDER=gemini`. |
| `AGENT_MAX_LLM_REQUESTS_PER_RUN` | `220` | no | **The bound that binds on a free-tier key** — matters most under `LLM_PROVIDER=gemini`, where there is no bill to cap and the scarce resource is requests per day (ADR-080). Under Anthropic, `AGENT_MAX_COST_USD_PER_RUN` below is the binding bound. |
...
2. `.env.example` **is** committed, with every variable name and empty values, so a future session knows what's needed without knowing any value:
   ...
   ```
   (`GEMINI_API_KEY` joins this list only if `LLM_PROVIDER=gemini` is set — see §3.)
...
4. All LLM calls originate in `apps/api`. There is **no** proxy route that forwards a client-supplied key, and no endpoint accepts a key in a request body or header.
5. The key is never logged. The provider's client is constructed once at startup, chosen by `LLM_PROVIDER`; log lines reference `LLM_EXPLAIN_MODEL` / `LLM_AGENT_MODEL`, never the key, and error handlers must not dump the client's config object.
...
7. **If a key is ever committed:** rotate it at the provider (Anthropic Console, or Google AI Studio for `GEMINI_API_KEY`) first, then worry about git history. Rewriting history without rotating is theatre — the key is already public.
