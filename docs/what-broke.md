...
- **2026-08-31** — **Day 12, U12: the Analyst's tool registry. Nine read-only tools, and the read-only claim is now enforced by Postgres rather than by this repo's good intentions.** Branch `day12-explain-layer`.
  ...
  **Also noticed, not acted on:** a `GEMINI_API_KEY` is now present in `apps/api/.env`, so `/api/health` reports `llmConfigured: true` where it reported `false` earlier today. No live call has been made from this session. Every test still exercises the template path, because they call `executeRun` without explain deps.
