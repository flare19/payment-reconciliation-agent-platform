...
- 2026-09-03: a citation in a saved Q&A answer linked to NOT FOUND. On run `5f2536c8`, "what is the largest single amount the engine could not match?" cited exception `0e37d78a` and transaction `eaa8b2e9`; `AskAboutRun` linked both to `/records/:id`, so the exception one 404'd — and dropped `?run=`, so even the right path would have hit the wrong run. The exception-detail panel had already solved this exact thing (`resolveCitation`, transaction→`/records/`, exception→`/exceptions/`, with a comment recording that the first all-`/records/` version sent a third of citations to not-found); the Q&A box shipped later (U15) and never picked it up. Fixed by resolving history citations server-side in `analyst/page.tsx` and handing `AskAboutRun` the id→kind map — correct links with real labels in the first paint, no client fetch, same as the panel. A just-asked answer's ids aren't in that map, so those few resolve client-side, rendering as plain text until they land rather than as the `/records/:id` guess. Verified live: `0e37d78a` now lands on the "Missing in Gateway" detail page. ADR-160.

  Fourth in the family this session — a later surface missing a fix an earlier one has (ADR-149, ADR-151, ADR-157), here a resolution step rather than a bug. All found by clicking the running product, none visible in the diff that introduced the gap.

- 2026-09-03, later: an external judge reviewed the localhost instance under an explicit "no benefit of the doubt, reproduce every claim or mark it UNPROVEN" brief, with no access to source. **Every headline accuracy claim survived** — precision `1.0000`/FP 0 re-derived on two independent seeds with separate answer keys, a fresh run reproduced all counts and input hashes byte for byte, the population arithmetic reconciled exactly (573 matched + 216 review + 85 neither = 874; 99 + 19 + 85 + 9 = 212 exceptions), and a deliberately crippled `fuzzyAutoConfirmThreshold: 0.99` probe held precision at 1.0 while collapsing the rate to 26.89%. The review found four defects, none in the engine and all in what the project **says about itself**, which for this project is the expensive place to be wrong.

  **`aliasLearningEnabled: false` was inert.** Accepted by the route, written verbatim into `config_snapshot` as `false`, and read by no code: the run loaded aliases anyway and came back warm — 65.56%, `isCold: false`, 2 aliases active — while its own snapshot said the feature was off. api-contract §2 names this field as *the* documented way to measure the cold-start rate, so the published route to the number this project is proudest of silently returned the other number. Fourth instance of the shape CLAUDE.md §10 names, and the first to land on a load-bearing claim. Fixed by gating the alias load; every downstream consumer already derives coldness from the alias set rather than the flag, so `aliasCountAtStart → 0` and `isCold → true` follow with no other change. The regression test was **watched failing against the unfixed engine** (`actual: 1, expected: 0`) before being trusted — the guard-must-fail rule, applied to the knob-test rule. Live: `65.22%`, `isCold: true`, matching the internally-computed cold rate exactly, so the two paths now agree. ADR-027 is untouched: no threshold, window, weight or tolerance moved, and the default run's numbers are byte-identical after the fix.

  **The contract described two things the build does not do.** Multipart upload (§2 Variant A) is documented as the primary way to start a run and returns `400 "file upload is not enabled on this build"`; endpoint 28's `toolCalls[]` is a `number` in the code, consistently, everywhere. Both stood without an ADR, which meant the repo's own binding rule — code and contract disagreeing means the code is wrong — declared the build broken in two places a reviewer reaches with `curl`. ADR-161 cuts Variant A explicitly, adopts the code's `toolCalls`, and the README now states the upload limit in the same breath as the throughput number rather than leaving it to be discovered at a 400.

  **`byDifficulty.precision` was recall wearing precision's name.** The scorer emitted `precision` holding the identical expression as `recall`, with a comment correctly explaining that precision is not sliceable by difficulty — but the payload said `precision` and a consumer reads the key, not the comment. It also published an impossible claim: engine precision is 1.0000 with 0 false positives, so a per-difficulty precision of 0.1504 cannot exist. In the `measured` block, of all places. The key is dropped rather than computed (scorer 1.5.0); a number that cannot be computed should be absent, not aliased onto its neighbour.

  **Two smaller ones, same family.** The Analyst page rendered "a verdict that cites something it never saw is rejected — 0 were" on any run with no investigations: a vacuous zero dressed as evidence, and indistinguishable from an honest one, on runs where the gate has in fact fired four times. Now scoped to the run, always carrying its denominator, and saying plainly when there is nothing to report. And a malformed id returned `500 INTERNAL_ERROR` — `GET /api/runs/not-a-uuid/metrics` reached Postgres, which rejected it with `22P02`, which is not an `ApiError` and so surfaced as an unknown throw. A client's typo is a client error: `uuidParam` now validates shape at the route across all 25 id path params, and a well-formed-but-absent id still correctly 404s.

  > The pattern across all four: **the engine was right and the paperwork was wrong.** Nothing here changed a match, a category or a score — the canonical run before and after is identical to four decimal places on every measured figure. What changed is that four claims a reviewer can check now match what the system does. For a submission whose entire argument is *trust the number because we tell you where it came from*, a stale README and an inert documented knob cost more than a missing feature would.

  Found the same way as the four before it: by exercising the running product against an expectation formed outside it. None was visible in the diff that introduced it, and the two that mattered most were invisible to 899 passing tests — one because no test asserted the field *did* anything, one because the only assertion was a comment.

- 2026-09-03, later still: built the balance proof (ADR-162) and **it found two real defects on its first sweep**, plus one in itself. The panel recomputes five identities from base rows on every request and is able to say no — the same stance as `/audit/verify`. Running it over all 39 stored runs was the first thing done with it.

  **A defect in the check itself, found before shipping.** C5 compared the S14 headline — frozen when the run finished — against a live recomputation that included human approvals. On the `verify` run, where a reviewer had approved 24 proposals, it reported the correct published `570` as mismatched against `640`, and would have rendered a red "the books do not balance" banner earned by using the product exactly as intended. That is worse than no check: it teaches a reader to ignore the one panel that must never be ignored. C5 now compares engine-alone to engine-alone (ADR-119's split) and reports the human contribution beside it. Three regression tests, one of which confirms the relaxation did not remove the teeth — C5 still fails when the engine figure itself is wrong.

  **An orphaned record on the dev dataset, in every one of 9 runs.** Bank row 64, `e0gt9CeMqbPHRKbMFVZD4`, **₹4,75,201.95**, `IMPS-SETL-FLIPKART INTERNET-BATCH47`, dated **2026-08-27 — the run's own reference date**. It is `status_norm = 'reconcilable'`, so it sits in the denominator dragging the match rate down; it is in no match, not even a proposal; and it is on no exception list. A controller reading every screen this product has would never see it. The reference-date coincidence is the obvious first place to look — a settlement dated on the boundary that some "not yet due" path skips without also removing it from the denominator. **The holdout seed is unaffected and every holdout run balances 5/5, so no reported number is wrong** — but the dev dataset has been quietly carrying an unaccounted ₹4.75 lakh, and nothing in the product could have told us.

  **Three more on `phase4-free`, from the reject path.** Rejecting a match returns its members to the pool, and afterwards no exception names them — `related_transaction_ids` does not cover them either. Endpoint 11's contract says a rejection creates an exception, and for a three-member group that is one exception for three records.

  > The shape worth naming: **a dropped record makes every other number look better.** It leaves the denominator alone and removes itself from the exception list, so match rate, precision and the category counts all improve slightly. It is the one class of error this project's entire measurement apparatus — an offline scorer, a published ceiling, a false-despair rate — could not see, because every one of those instruments reads the engine's *output* rather than asking whether the output covers the input. Six defects this week were found by exercising the running product against an expectation formed outside it. This one was found by asking the product to account for itself.

  Both engine defects are left open and surfaced rather than fixed on the way past: each changes engine behaviour, and both deserve a deliberate decision rather than a drive-by. The panel reporting them is the feature working.

- 2026-09-03, and this is what the balance proof was for: **both orphan defects it found turned out to be the same bug — a decision the engine made correctly and left no trace of** (ADR-163). Neither was a wrong answer. Both were real states the product never named, which is exactly why nothing else caught them: the offline scorer, the published ceiling and the false-despair rate all read the engine's OUTPUT, and not one of them asks whether the output covers its input.

  **The dev-seed record.** S12's presence rule reads `if (!due.overdue) continue` — a bank credit dated on the run's own reference date has not had time to reach the ledger, so calling it `MISSING_IN_LEDGER` would be a false exception, the confident wrong answer this whole engine is built to refuse. Right call. But a record where *every* open target is still in flight picks up no signal at all: it stayed in the match-rate denominator dragging the rate down, and appeared on no screen a human reads. On the dev dataset that was ₹4,75,201.95, in every run, for as long as the dataset has existed.

  **The rejection records.** Endpoint 11 returns a rejected match's members to the pool and raises nothing, with a comment that re-classifying is S12's job rather than something a route improvises. That reasoning holds — an exception invented in a route carries no precedence. The cost was three records unaccounted for until someone re-ran.

  Fixed by naming the states, not by changing the answers: migration 014 adds `transactions.deferred_reason`, S12 writes it in the same transaction as the exceptions, and endpoint 29 counts `notYetDue` from that column plus `awaitingReclassification` straight off `matches.status`. C3 now admits three legitimate ends for an unresolved record and **still fails on one in none of them** — it compares both sides for equality, so a deferral can never absorb more than the unresolved population, and a run claiming 90 deferred out of 85 fails. Four tests hold that line, including one that exists purely to prove the fix did not define the problem away.

  Verified: dev seed reads `79 unresolved = 78 on the exception list + 1 not yet due` and balances 5/5; `phase4-free` reads `88 = 85 + 0 + 3 awaiting re-classification` with no re-run needed. **No engine number moved** — dev seed still 64.61% / 198 exceptions / 876 reconcilable, holdout still 65.56% / 212 / 874, precision 1.0000, scorer exit 0. The nine dev-seed runs created before the migration still show the orphan and correctly so: the column records what S12 decided, and S12 did not run again.

  > The rejected alternative is worth recording. Excluding not-yet-due records from the denominator would also have balanced the books — and would have *raised* the match rate. That is the direction ADR-027 exists to be suspicious of, and the record genuinely is reconcilable; it simply is not due. Leaving it in keeps the rate slightly pessimistic, which is the right way for this project to be wrong.

- 2026-09-04: **the eighth exception category has never once fired, on any dataset, in any run — and the engine is not the reason** (ADR-169). Noticed from the dashboard: the taxonomy declares eight categories and the breakdown draws seven.

  The first read was an engine defect, since `TIMING_DRIFT` is one of the two categories ADR-029 created S8 specifically to make reachable, and the other one (`AMOUNT_MISMATCH`) fires 36 times. It is not. S8 computes `outcome: 'timing_drift'` correctly, `dedupe-identity.test.ts` asserts it on a +9-day settlement and passes, S12 emits it verbatim, and precedence, severity, the wire DTO, the label table and the scorer's confusion matrix all carry it. Every line of the path works.

  **The generator never produces its input.** `normalLagDays` draws 0–1; `edgeLagDays` draws 2–3 for card against a `[-1, 3]` window and 1–2 for UPI against `[-1, 2]` — the inside edge, on purpose. `TIMING_LAG_NORMAL`'s own note says it: *"Settlement lands inside the declared window; a match, not a drift exception."* No scenario in the table plants a late settlement, so the drift rule is a correct rule with no input, and has been since the dataset was first generated.

  Two things fell out of the same fact. `TIMING_LAG_NORMAL` sets `bankAnchorExtractable: false`, so **widening its lag would not have helped** — S8 needs a strong anchor structured on both sides, that pair goes to Tier 2, and a large date delta there lowers the score into a presence exception. It would have cost recall and still produced no drift. And the §5.2 BUILD BLOCKER *"TIMING_DRIFT auto-confirmed"* keys on `expectedCategory === 'TIMING_DRIFT'`, which no event in any answer key has: **a blocker that is wired, documented, and structurally unable to fire.** CLAUDE.md §9's "a guard nobody has watched fail" — the same shape as the four parsed-but-unenforced config knobs, arrived at from the other direction.

  Not fixed, deliberately, and the measurement is why. `planEvents` shuffles the allocated scenario slots and draws each event's amount and date by shuffled index, so **changing any weight changes which scenario lands at every index** — adding a scenario is not additive, it regenerates all three seeds wholesale. That means 9 hash-pinned fixtures, 3 answer keys, **83 quotations of the headline figures across 26 files**, a stale Railway run and score report, and a re-filled explain cache. Set against a taxonomy display that would read 8/8 instead of 7/8. Precision 1.0000 with FP 0 is a property of *this* dataset as much as of the engine, and reshuffling 300 events the day before submission risks it with no time to recover — which is the exact move ADR-027 forbids, pointed at the data instead of a threshold.

  What shipped is the honest version: the breakdown names the absent categories under the list — *"Zero, not missing. The rule is wired and unit-tested; no record in this dataset met its definition."* Derived from the label table, so a category that empties on a future dataset names itself.

  > The shape: **an empty result and an absent one render identically, and the API cannot tell them apart either.** `byCategory` only carries keys that occurred, so a category no record met simply falls out of the payload — there is no zero to read anywhere in the stack. Seven bars where the docs promise eight is not a lie, but it leaves a reviewer to guess between "unwired", "broken" and "genuinely empty", and only one of those is true. Same family as the vacuous-zero fix on the Analyst page: the number was never wrong, the screen just wasn't saying which kind of nothing it was.

- 2026-09-04: a second external review found four blockers and **not one of them was in the build**. The six frontend items all held up under testing — the reviewer set `PINNED_RUN_ID` themselves and broke it deliberately, summed all 212 exceptions by hand, and recomputed all 612 audit hashes with an independently written serializer that matched the server's chain head exactly. What nearly shipped was the environment around the code.

  **The API was serving the database the tests destroy.** `apps/api/.env` pointed `DATABASE_URL` at `recon_test`, and nine integration files open with `TRUNCATE … CASCADE` against `TEST_DATABASE_URL ?? DATABASE_URL`. The obvious command — `TEST_DATABASE_URL="$(grep DATABASE_URL .env …)"` — therefore aimed the suite at the live application data. The reviewer had to create a scratch database to run the tests at all. Fixed at the source: one shared `test-db.ts` resolves the URL, `DATABASE_URL` is no longer a fallback, and a target whose database name does not end in `_test` **throws** rather than skipping. Watched refusing `recon_v2` before being trusted.

  **`pass 727, fail 0, skipped 0` had been meaning "no integration test ran".** The skip message now says `TEST_DATABASE_URL is not set — integration tests SKIPPED (this is not a pass)`. A green run that executed nothing is worse than a red one.

  **The demo database had 4 runs and 0 aliases** — two of them named `other` and `tools`, left by a test. The real 44 runs and both learned aliases were in `recon_v2`, which the app had stopped pointing at. Every run read as cold (65.22% instead of 65.56%) and **alias learning was undemonstrable**: empty aliases screen, no warm/cold comparison, no leverage ratio. Restored by repointing; 44 runs and 4 aliases are back, including the superseded `WRONG TARGET` row that shows supersession working.

  **The footer's `8.24 s` was a warm-cache number presented unconditionally.** Measured both ways on the same 920 records: explain stage **33,948 ms** cold against **117 ms** warm, wall clock **26 rec/s** against **377 rec/s**, and the matching engine identical either way (Tier 2, 211 ms both times). A judge clicking "Run It Again — Free" on a freshly deployed database would have waited half a minute under a figure promising eight seconds. The footer now states the condition, and §5.2 gains an explicit cache-warming step — the seed run *is* the warming run, so it is the slow one and every run after it costs $0.

  **`score:watch` was a laptop process.** §5.5 makes it a third Railway service, with the reason it cannot live inside the API written down: `tools/score` reads `data/truth/`, and ADR-021 forbids anything under `apps/api` from importing that. The boundary is why the accuracy numbers mean anything, so the scorer stays one HTTP hop away and gets no database credentials.

  > And a correction to the first review, recorded because getting it wrong in writing deserves getting it right in writing. That review reported the Analyst as "zero proposals in sixteen investigations" and read it as zero usefulness. `proposals` counts only `RESOLUTION_PROPOSED`; agent-design.md designs three other terminal verdicts, and says of one of them that *"in real finance ops it is the most common honest answer, and an agent that cannot say it will fabricate one of the other three."* Counting `NEEDS_EXTERNAL_DATA` and `CONFIRMED_UNRESOLVABLE` as failures inverted the design. The narrow claim survives — no persisted investigation has reached `RESOLUTION_PROPOSED`, so the one-click accept path is unexercised — but that is a coverage gap, not a verdict on the feature.
